...
void UpdateEmployee() {
  ...
  int peopleIndex = FindPerson(TargetName);
  ...
  if (peopleIndex != -1) {
    ...
    switch (update_choice) {
      ...
      case 2:
        ...
        strcpy_s(EmployeeTable[peopleIndex].residence, 40, NewResidence);
        break;
      case 3:
        printf("새로운 직급 입력: ");
        scanf_s("%s", NewRole, (int)sizeof(NewRole));
        strcpy_s(EmployeeTable[peopleIndex].role, 40, NewRole);
        break;
      case 4:
        printf("새로운 근무 연도 입력: ");
        scanf_s("%s", Newstart_company, (int)sizeof(Newstart_company));
        strcpy_s(EmployeeTable[peopleIndex].start_company, 40,Newstart_company);
        break;
      default:
        printf("잘못된 선택입니다.\n");
    }
